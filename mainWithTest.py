...
def move(matrix, population, generations, max, initRoad):
    ...
    for currentGeneration in range(1, generations):
        os.system('cls')

        print('Geração: {}'.format(currentGeneration))
        bestIndividual.setGeneration(currentGeneration)

        bestIndividual.generateIndexesArrays(initialCoordinates, maze)
        print('Possiveis caminhos: {}'.format(bestIndividual.possibleRoads))
        print('Indices de cruzamento: {}'.format(
            bestIndividual.indexesOfMultipleRoads))
        bestIndividual.appendRoad(bestIndividual.possibleRoads[-1])
        print('Caminho: {}'.format(bestIndividual.road))

        bestIndividual.fitness(initialCoordinates, maze)

        bestIndividual.mutation(5, 50)
    ...
    # bestIndividual.moveChromossome(initialCoordinates, matrix)
    # showWindow(matrix)

    return {
        "Encontrou": bestIndividual.found,
        "Geração": bestIndividual.generation,
        "Tempo": timer() - start
    }
...
for _ in range(numberOfRounds):
    ...
    population = Population(initialPoint, matrix, initRoadSize, populationSize)
    rounds.append(move(matrix, population, generations, 100000, initRoadSize))
